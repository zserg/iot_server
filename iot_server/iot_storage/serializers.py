    ...
    def validate(self, attrs):
        ...
        if node:
            if self.get_data_type(value) != node.data_type:
                raise DataWriteError("Write data type doesn't match the datanode data type")
...
class DataReadSerializer(serializers.ModelSerializer):
    ...
    def get_datapoints(self,obj):
        ...
        if fromdate and not todate:
            dps = Datapoint.objects.filter(node=obj, created_at__gte=fromdate).order_by(order)
        elif fromdate and todate:
            dps = Datapoint.objects.filter(node=obj, created_at__gte=fromdate,
                                           created_at__lte=todate).order_by(order)
        else:
        ...
        # count = dps.count()
        # if
        serializer = DatapointReadSerializer(dps[offset:offset+limit], many=True)
